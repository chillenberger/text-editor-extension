...
export class CoDocView implements WebviewViewProvider {
  ...
  constructor(private readonly _extensionUri: Uri, planningService: PlanningService, userState: UserState, private setUserState: (newState: UserState) => void) {
    ...
    this._systemPrompt = userState.systemPrompt ?? "";
  }
        ...
        try {
          switch (message.command) {
            ...
            case "setSystemPrompt": 
              if (message.text !== undefined) {
                this._handleSystemPromptUpdate(message.text);
              }
              break;
            case "refresh":
  ...
  private async _handleChatMessage(instruction: string) {
    try {
      ...
      const responses = await this.planningService.executePlanningLoop({
        ...
        // specialInstructions: this._systemPrompt
      });
  ...
  private async _handleSystemPromptUpdate(newPrompt: string) {
    this._systemPrompt = newPrompt;
    this._persistUserState();
    this.sendMessage({
      type: "systemPromptSaved",
      data: { text: this._systemPrompt },
    });
  }
